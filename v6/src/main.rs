...
fn main() {
    ...
    let sun_dir: Vec3 = Vec3::new(0.15, 0.4, -0.5).normalize();
    let sky_turbidity: f32 = 3.0;
    let ground_albedo: (f32, f32, f32) = (0.5, 0.1, 0.7);

    // Build the image in a benchmark loop
    let mut pixels: Vec<u8> = Vec::<u8>::new();
    let mut durations: Vec<std::time::Duration> = Vec::<std::time::Duration>::new();
    let runs: usize = 10;
    for _ in 0..runs {
        let now = std::time::Instant::now();
        pixels = build_zneg_face(sun_dir, sky_turbidity, ground_albedo, width, height);
        durations.push(now.elapsed());
    }

    // Report
    let average_duration: std::time::Duration =
        durations.iter().skip(2).take(runs - 4).sum::<std::time::Duration>() / (runs - 4) as u32;
    println!("Average duration: {:?}", average_duration);

    // Save the image
    let image: ImageBuffer<Rgb<u8>, _> = ImageBuffer::from_raw(width as u32, height as u32, pixels).unwrap();
    image.save("skybox_zneg_face.png").unwrap();
}
...
fn build_zneg_face(
    ...
    let sun_theta: f32 = sun_direction.y.acos(); // angle from zenith, radians
    let sun_elevation: f32 = (std::f32::consts::PI / 2.0 - sun_theta).max(0.0); // angle from the horizon, radians
    let sky: HosekWilkieSky = HosekWilkieSky::new(sky_turbidity, ground_albedo, sun_elevation); // the sky model
    let tone_mapper = ReinhardToneMapper::new(0.5, 14.0);

    // Set up the initial direction vector for y=0/x=0, face=zneg.
    let mut dir_row: Vec3 = Vec3::new(-1.0, 1.0, -1.0);
    let dir_dy: f32 = -1.0 / (height as f32); // Set up the Y-direction increment for each row.
    let dir_dx: f32 = 2.0 / (width as f32); // Set up the X-direction increment for each column.
    let dir_dx_x_4: F32x4 = F32x4::splat(dir_dx) * F32x4::splat(4.0);
    let dir_offset_x_4: F32x4 = F32x4::load([dir_dx, dir_dx * 2.0, dir_dx * 3.0, dir_dx * 4.0]);
    let sun_dir_x_4: F32x4 = F32x4::splat(sun_direction.x);
    let sun_dir_y_4: F32x4 = F32x4::splat(sun_direction.y);
    let sun_dir_z_4: F32x4 = F32x4::splat(sun_direction.z);
    ...
    for y in 0..height {
        ...
        let vec_y_4: F32x4 = F32x4::splat(dir_row.y);
        let vec_y2_4: F32x4 = vec_y_4 * vec_y_4;
        let vec_z_4: F32x4 = F32x4::splat(dir_row.z);
        let vec_z2_4: F32x4 = vec_z_4 * vec_z_4;
        let vec_y2_z2_4: F32x4 = vec_y2_4 + vec_z2_4;
        for x in (0..width).step_by(4) {
            // normalize the components of the direction vector
            // let recip_len_sqrt: F32x4 = (vec_x_4 * vec_x_4 + vec_y2_z2_4).rsqrt();
        ...
        // Map the radiance values to RGB colors and store them in the image.
        tone_mapper.map(&r_row, &g_row, &b_row, pixels[y * width * 3..y * width * 3 + width * 3].as_mut(), y);
    }
    pixels
}
...
impl HosekWilkieSky {
    // turbidity: amount of moisture/aerosols in the atmosphere, [1.0...10.0]
    // albedo: ground albedo/reflectance per channel, [0.0...1.0] x 3
    // elevation: angle between sun and horizon in radians, [0...Pi/2]
    pub fn new(turbidity: f32, albedo: (f32, f32, f32), solar_elevation: f32) -> Self {
        debug_assert!(turbidity >= 1.0 && turbidity <= 10.0);
        debug_assert!(albedo.0 >= 0.0 && albedo.0 <= 1.0);
        debug_assert!(albedo.1 >= 0.0 && albedo.1 <= 1.0);
        debug_assert!(albedo.2 >= 0.0 && albedo.2 <= 1.0);
        debug_assert!(solar_elevation >= 0.0 && solar_elevation <= std::f32::consts::PI / 2.0);
        let x: f64 = (solar_elevation as f64 / (3.14 / 2.0)).powf(1.0 / 3.0);
        let mut sky = Self { radiance: [0.0; 3], distribution: [[0.0; 9]; 3] };
        sky.radiance[0] = Self::radiance(RADIANCE_PARAMETERS_1, turbidity as f64, albedo.0 as f64, x);
        sky.radiance[1] = Self::radiance(RADIANCE_PARAMETERS_2, turbidity as f64, albedo.1 as f64, x);
        sky.radiance[2] = Self::radiance(RADIANCE_PARAMETERS_3, turbidity as f64, albedo.2 as f64, x);
        sky.distribution[0] = Self::distribution(DISTRIBUTION_PARAMETERS_1, turbidity as f64, albedo.0 as f64, x);
        sky.distribution[1] = Self::distribution(DISTRIBUTION_PARAMETERS_2, turbidity as f64, albedo.1 as f64, x);
        sky.distribution[2] = Self::distribution(DISTRIBUTION_PARAMETERS_3, turbidity as f64, albedo.2 as f64, x);
        sky
    }
    ...
    fn f_simd_channel<const CHANNEL: usize>(
        ...
        debug_assert!(gamma.len() == theta_cos.len() && gamma.len() == gamma_cos.len() && gamma.len() == output.len());
        debug_assert!(gamma.len() % 4 == 0);
        ...
        let b: F32x4 = F32x4::splat(self.distribution[CHANNEL][1]);
        let c: F32x4 = F32x4::splat(self.distribution[CHANNEL][2]);
        let d: F32x4 = F32x4::splat(self.distribution[CHANNEL][3]);
        let e: F32x4 = F32x4::splat(self.distribution[CHANNEL][4]);
        let f: F32x4 = F32x4::splat(self.distribution[CHANNEL][5]);
        let g: F32x4 = F32x4::splat(self.distribution[CHANNEL][6]);
        let h: F32x4 = F32x4::splat(self.distribution[CHANNEL][7]);
    ...
    pub fn f_simd_r(&self, gamma: &[f32], theta_cos: &[f32], gamma_cos: &[f32], output: &mut [f32]) {
        self.f_simd_channel::<0>(gamma, theta_cos, gamma_cos, output);
    }

    pub fn f_simd_g(&self, gamma: &[f32], theta_cos: &[f32], gamma_cos: &[f32], output: &mut [f32]) {
        self.f_simd_channel::<1>(gamma, theta_cos, gamma_cos, output);
    }

    pub fn f_simd_b(&self, gamma: &[f32], theta_cos: &[f32], gamma_cos: &[f32], output: &mut [f32]) {
        self.f_simd_channel::<2>(gamma, theta_cos, gamma_cos, output);
    }

    fn sample(p: [f64; 6], x: f64) -> f64 {
        let one_minus_x = 1.0 - x;
        p[0] * one_minus_x.powi(5)
            + p[1] * 5.0 * x * one_minus_x.powi(4)
            + p[2] * 10.0 * x.powi(2) * one_minus_x.powi(3)
            + p[3] * 10.0 * x.powi(3) * one_minus_x.powi(2)
            + p[4] * 5.0 * x.powi(4) * one_minus_x
            + p[5] * x.powi(5)
    }

    fn radiance(table: [f64; 120], turbidity: f64, albedo: f64, x: f64) -> f32 {
        assert!(albedo >= 0.0 && albedo <= 1.0);
        assert!(turbidity >= 1.0 && turbidity <= 10.0);
        assert!(x >= 0.0 && x <= 1.0);
        // table: [2 x albedo][10 x turbidity][6 x values]
        let int_turbidity: usize = turbidity as usize;
        let turbidity_rem: f64 = turbidity - int_turbidity as f64;
        let mut result: f64 = 0.0;

        // low albedo, low turbidity
        let idx: usize = 6 * (int_turbidity - 1);
        let slice: [f64; 6] = table[idx..idx + 6].try_into().expect("Wrong offset");
        result += (1.0 - albedo) * (1.0 - turbidity_rem) * Self::sample(slice, x);

        // high albedo, low turbidity
        let idx: usize = 6 * 10 + 6 * (int_turbidity - 1);
        let slice: [f64; 6] = table[idx..idx + 6].try_into().expect("Wrong offset");
        result += (albedo) * (1.0 - turbidity_rem) * Self::sample(slice, x);

        if int_turbidity < 10 {
            // low albedo, high turbidity
            let idx: usize = 6 * int_turbidity;
            let slice: [f64; 6] = table[idx..idx + 6].try_into().expect("Wrong offset");
            result += (1.0 - albedo) * (turbidity_rem) * Self::sample(slice, x);

            // high albedo, high turbidity
            let idx: usize = 6 * 10 + 6 * int_turbidity;
            let slice: [f64; 6] = table[idx..idx + 6].try_into().expect("Wrong offset");
            result += (albedo) * (turbidity_rem) * Self::sample(slice, x);
        }
        result as f32
    }

    fn distribution(table: [f64; 1080], turbidity: f64, albedo: f64, x: f64) -> [f32; 9] {
        assert!(albedo >= 0.0 && albedo <= 1.0);
        assert!(turbidity >= 1.0 && turbidity <= 10.0);
        assert!(x >= 0.0 && x <= 1.0);
        // table: [2 x albedo][10 x turbidity][9 x parameters][6 x values]
        let int_turbidity: usize = turbidity as usize;
        let turbidity_rem: f64 = turbidity - int_turbidity as f64;
        ...
        // low albedo, low turbidity
        let idx: usize = 9 * 6 * (int_turbidity - 1);
        let slice: [f64; 54] = table[idx..idx + 54].try_into().expect("Wrong offset");
        for i in 0..9 {
            let p: [f64; 6] = [slice[i + 0], slice[i + 9], slice[i + 18], slice[i + 27], slice[i + 36], slice[i + 45]];
            result[i] += (1.0 - albedo) * (1.0 - turbidity_rem) * Self::sample(p, x);
        }

        // high albedo, low turbidity
        let idx: usize = 9 * 6 * 10 + 9 * 6 * (int_turbidity - 1);
        let slice: [f64; 54] = table[idx..idx + 54].try_into().expect("Wrong offset");
        for i in 0..9 {
            let p: [f64; 6] = [slice[i + 0], slice[i + 9], slice[i + 18], slice[i + 27], slice[i + 36], slice[i + 45]];
            result[i] += albedo * (1.0 - turbidity_rem) * Self::sample(p, x);
        }

        if int_turbidity < 10 {
            // low albedo, high turbidity
            let idx: usize = 9 * 6 * int_turbidity;
            let slice: [f64; 54] = table[idx..idx + 54].try_into().expect("Wrong offset");
            for i in 0..9 {
                let p: [f64; 6] =
                    [slice[i + 0], slice[i + 9], slice[i + 18], slice[i + 27], slice[i + 36], slice[i + 45]];
                result[i] += (1.0 - albedo) * turbidity_rem * Self::sample(p, x);
            }

            // high albedo, high turbidity
            let idx: usize = 9 * 6 * 10 + 9 * 6 * int_turbidity;
            let slice: [f64; 54] = table[idx..idx + 54].try_into().expect("Wrong offset");
            for i in 0..9 {
                let p: [f64; 6] =
                    [slice[i + 0], slice[i + 9], slice[i + 18], slice[i + 27], slice[i + 36], slice[i + 45]];
                result[i] += albedo * turbidity_rem * Self::sample(p, x);
            }
        }

        let mut result_f32 = [0.0f32; 9];
        for i in 0..9 {
            result_f32[i] = result[i] as f32;
        }
        result_f32
    }
}
...
static RADIANCE_PARAMETERS_1: [f64; 120] = [
...
static RADIANCE_PARAMETERS_2: [f64; 120] = [
    // albedo 0, turbidity 1
    1.590330e+000,
    1.355401e+000,
    1.151412e+000,
...
static RADIANCE_PARAMETERS_3: [f64; 120] = [
    // albedo 0, turbidity 1
    9.926518e-001,
    1.999494e+000,
    -4.136109e+000,
...
static DISTRIBUTION_PARAMETERS_1: [f64; 1080] = [
    // albedo 0, turbidity 1
    -1.099459e+000,
    -1.335146e-001,
    -4.083223e+000,
...
static DISTRIBUTION_PARAMETERS_2: [f64; 1080] = [
    // albedo 0, turbidity 1
    -1.140530e+000,
    -1.982747e-001,
    -7.512730e+000,
...
static DISTRIBUTION_PARAMETERS_3: [f64; 1080] = [
    // albedo 0, turbidity 1
    -1.372629e+000,
    -4.905585e-001,
    -4.100789e+001,
